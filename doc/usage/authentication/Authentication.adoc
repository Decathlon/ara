...
. Create the file `code/api/server/src/main/resources/config-authentication.yaml`. Note that this file is ignored, preventing you from submitting your credentials by mistake
. Copy the example file (`code/api/server/src/main/resources/config-authentication.yaml.example`) content in the newly created file
