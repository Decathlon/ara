...
=== Back authentication
Like the front authentication, you can configure the back authentication as well. In fact, you can update the `application.properties` values:
...
****
** `*authentication.jwt-token.access-token-expiration-in-second*`: your token validity duration (in seconds). This field is *mandatory* (if a token has a duration of 0 second, then it's not valid!)

TIP: This file is found in `code/api/server/src/main/resources/application.properties` in your project. You can also pass the values as parameters (*highly recommended* on production environment, especially for sensitive data related to authentication!).
