...
IMPORTANT: The front authentication configuration being stored on the server side, it will be described below.
...
** `*authentication.front-enabled*`: enable (`true`) or disable (`false`) the *front* authentication.
+
WARNING: You have to explicitly set the value to true to enable the back or front authentication. If left empty, it is considered disabled.
* *OAuth2 providers*:
** *Github*:
*** `*authentication.clients.github.front-enabled*`: a boolean to enable or disable the Github *front* authentication
*** `*authentication.clients.github.client-id*`: the Github client id
*** `*authentication.clients.github.client-secret*`: the Github client secret
** *Google*:
*** `*authentication.clients.google.front-enabled*`: a boolean to enable or disable the Google *front* authentication
*** `*authentication.clients.google.client-id*`: the Google client id
*** `*authentication.clients.google.client-secret*`: the Google client secret
** *Custom*:
*** `*authentication.clients.custom.front-enabled*`: a boolean to enable or disable the custom *front* authentication
*** `*authentication.clients.custom.displayed-name*`: the name you want to display (e.g. authentication button, popover, etc.). It can be your company name. If none, the value `Custom` is then displayed
*** `*authentication.clients.custom.login-uri*`: the uri used by a browser to login and get the code
+
****
